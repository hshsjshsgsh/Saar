...
async def log_reward_update(guild_id, user_id, rp_gained=0, crowns_gained=0):
    ...
    if guild_str in log_channels:
        ...
        if channel:
            ...
            # Also log the specific update
            guild = bot.get_guild(guild_id)
            user = guild.get_member(int(user_id)) if guild else None
            if user:
                update_text = f"{user.display_name} gained: "
                if rp_gained > 0:
                    update_text += f"{rp_gained}<:Ranked:1411317994847473695> "
                if crowns_gained > 0:
                    update_text += f"{crowns_gained}<:Crown:1394255336310968434> "
                if guild_str in bracket_roles and str(user_id) in bracket_roles[guild_str]:
                    emojis = ''.join(bracket_roles[guild_str][str(user_id)])
                    update_text += f"\u23f1️ {emojis}"
