...
log_message_ids = {}  # {guild_id: message_id} for persistent leaderboard messages
tournaments = {}  # {guild_id: Tournament}
role_permissions = {
...
def load_data():
    ...
    try:
        ...
        print("⚠️ No existing data file found, starting fresh")
        ...
        try:
            ...
            print("❌ Could not load backup data, starting fresh")
...
async def update_log_embed(guild_id, channel):
    """Update or create log embed with current RP and crown leaderboard"""
    guild_str = str(guild_id)
    ...
    if not combined_data:
        embed.description = "No players with RP or crowns yet."
    else:
        leaderboard_text = ""
        for i, (user_id, rp, crowns) in enumerate(combined_data[:10], 1):
            ...
            if user:
                ...
                line = f"{emoji} {user.display_name} - {rp}<:Ranked:1411317994847473695>"
                if crowns > 0:
...
🆘 Need help? Contact support immediately."""
        ...
        embed = discord.Embed(description=embed_description, color=0x00ff00)
...
async def rp_rst(ctx):
    ...
    save_data()
    await ctx.send(
        "✅ All Ranked Points and Crowns have been reset for this server!",
        delete_after=5)
...
async def rp_add(ctx, member: discord.Member, amount: int):
    ...
    await ctx.send(f"✅ Added {amount}<:Ranked:1411317994847473695> to {member.display_name}!", delete_after=5)

@bot.command()
@commands.has_permissions(administrator=True)
async def rp_rmv(ctx, member: discord.Member, amount: int):
    try:
        await ctx.message.delete()
    except:
        pass
    ...
    rp_data[guild_str][user_str] = max(0, rp_data[guild_str][user_str] - amount)
    save_data()
    await log_reward_update(ctx.guild.id, member.id, -amount, 0)
    await ctx.send(f"✅ Removed {amount}<:Ranked:1411317994847473695> from {member.display_name}!", delete_after=5)
...
async def crwn_add(ctx, member: discord.Member, amount: int = 1):
    ...
    await ctx.send(f"✅ Added {amount}<:Crown:1394255336310968434> to {member.display_name}!", delete_after=5)
        ...
        for i, (user_id, crowns) in enumerate(sorted_players, 1):
            ...
            if user:
                ...
                if i == 1:
                    ...
                    emoji = f"**{i}.**"
                leaderboard_text += f"{emoji} {user.display_name} - {crowns}<:Crown:1394255336310968434>\n"
...
async def update(ctx):
    ...
    if guild_str in log_channels:
        ...
        channel = bot.get_channel(channel_id)
        if channel:
            # Update the embed with current leaderboard info
            await update_log_embed(ctx.guild.id, channel)
            await ctx.send("✅ Log embed updated!", delete_after=5)
            return
